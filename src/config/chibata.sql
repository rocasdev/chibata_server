...
BEGIN
    INSERT INTO users (
        name, surname, email, doc_type, doc_num, phone_number, pass, role_id, profile_photo
    ) VALUES (
        p_name, p_surname, p_email, p_doc_type, p_doc_num, p_phone_number, p_pass, p_role_id, p_profile_photo
    );
END //
...
CREATE PROCEDURE toggle_user_state (
    IN p_user_id BIGINT
)
BEGIN
    UPDATE users SET state = IF(state = true, false, true) WHERE user_id = p_user_id;
END //
